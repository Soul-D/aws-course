...
resource "aws_instance" "private_instance" {
  ...
  user_data = <<-EOF
    ...
    echo $RDS_HOST
    echo 'SELECT * FROM LOGS;' > select_logs.sql
    yum -y update 
    yum -y install java-1.8.0-openjdk
    amazon-linux-extras install -y postgresql13
    aws s3api get-object --bucket iam-ozinoviev-final-bucket --key persist3-2021-0.0.1-SNAPSHOT.jar persist3-2021-0.0.1-SNAPSHOT.jar
